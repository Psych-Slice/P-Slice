	...
	public function new(?extraMode:ExtraActions = NONE, colorMap:Array<FlxColor>)
	{
		...
		switch (extraMode)
		{
			...
			case ARROWS: // This is a special case
				...
				final ARROW_HITBOX_SIZE = 230;
				final ARROW_DISTANCE = 185;


				add(buttonLeft = createHint(SCREEN_MIDDLE-(ARROW_DISTANCE*1.5)-(ARROW_HITBOX_SIZE/2), 0, ARROW_HITBOX_SIZE, FlxG.height, colorMap[0]));
				add(buttonDown = createHint(SCREEN_MIDDLE-(ARROW_DISTANCE*0.5)-(ARROW_HITBOX_SIZE/2), 0, ARROW_HITBOX_SIZE, FlxG.height, colorMap[1]));
				add(buttonUp = createHint(SCREEN_MIDDLE+(ARROW_DISTANCE*0.5)-(ARROW_HITBOX_SIZE/2), 0, ARROW_HITBOX_SIZE, FlxG.height, colorMap[2]));
				add(buttonRight = createHint(SCREEN_MIDDLE+(ARROW_DISTANCE*1.5)-(ARROW_HITBOX_SIZE/2), 0, ARROW_HITBOX_SIZE, FlxG.height, colorMap[3]));
		}
