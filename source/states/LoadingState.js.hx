...
typedef PreloadTask<T> =
{
	var task:Void->Future<T>;
	var onComplete:T->Void;
	var onError:Void->Void;
}

class LoadingState extends MusicBeatState
{ // JS exclusive loading screen (it's too laggy on sys)
	public static var loaded:Int = 0;
	public static var loadMax:Int = 0;

	static var promises:Array<PreloadTask<Dynamic>> = [];
	static var loadedBitmaps:Array<String> = [];
	static var loadedSounds:Array<String> = [];
	...
	public static function prepareToSong(intrusive:Bool = true)
	{
		...
		for (x in loadedBitmaps) OpenFlAssets.cache.removeBitmapData(x);
		for (x in loadedSounds) OpenFlAssets.cache.removeSound(x);
	...
	static function _threadFunc()
	{
		new Future(() ->
		{
			...
			for (sound in soundsToPrepare)
				promises.push(preloadSound('sounds/$sound'));
			for (music in musicToPrepare)
				promises.push(preloadSound('music/$music'));
			for (song in songsToPrepare)
				promises.push(preloadSound(song, 'songs', true, false));

			// for images, they get to have their own thread
			for (image in imagesToPrepare)
				promises.push(preloadGraphic(image));
	...
	static function initThread(func:Void->Void, traceData:String)
	{
		...
		new Future(() ->
		{
			...
			try
			{
				...
				trace('finished requesting $traceData in ${diff}s');
				#end
			}
			catch (e:Dynamic)
			{
	...
	static function preloadSound(key:String, ?path:String, ?modsAllowed:Bool = true, ?beepOnNull:Bool = true):PreloadTask<Sound>
	{
		...
		// {
		// 	Paths.currentTrackedSounds.remove(file);
		// 	trace('$file was present, but let\'s recache it for good measure');
		// }
		if (OpenFlAssets.exists(file, SOUND))
		{
			return {
				...
				onError: () ->
				{
					trace('ERROR! fail on preloading $file ');
				},
				onComplete: (snd) ->
				{
